...
st.set_page_config(page_title="Medical Imaging Service Case Simulator",page_icon="🩻",layout="wide",)
...
def metrics_to_dataframe(metrics: dict[str, float]) -> pd.DataFrame:
    return pd.DataFrame([{"Metric":key, "Value": round(value, 3)} for key, value in metrics.items()])
st.title("🩻 Medical Imaging Service Case Simulator")
st.caption("Aplicație pentru analiză simplă de calitate a imaginii și raport QA-style." ,"Nu este instrument clinic validat.")
with st.sidebar:
    st.header("Input")
    input_mode=st.radio("Alege sursa imaginii:",["Încarcă imagine / DICOM", "Folosește phantom demo"],)
    uploaded_file =None
    selected_case="Normal"
    if input_mode == "Încarcă imagine / DICOM":uploaded_file = st.file_uploader("Încarcă fișier", type=["dcm", "dicom", "png", "jpg", "jpeg", "bmp", "tif", "tiff"],)selected_case = st.selectbox("Simulează o problemă peste imaginea încărcată:",["Normal", "Contrast scăzut", "Zgomot crescut", "Blur / mișcare", "Neuniformitate", "Artefact bandă"],)
    else:
        selected_case = st.selectbox("Alege scenariul demonstrativ:",["Normal", "Contrast scăzut", "Zgomot crescut", "Blur / mișcare", "Neuniformitate", "Artefact bandă"],)
    st.divider()
    st.markdown("### Praguri euristice")
    st.write( "Interpretarea este orientativă. Pentru QA real se folosesc phantom, protocoale, toleranțe ""și documentația producătorului.")
image = None
metadata = {}
...
if input_mode == "Încarcă imagine/DICOM":
    ...
    image = apply_case(image, selected_case)
    metadata = {"Source": "Synthetic phantom","Simulated case": selected_case,"Rows": str(image.shape[0]),"Columns": str(image.shape[1]),}
if image is not None:
    ...
    with st.expander("Metadata"):
        st.json(metadata)
    report_html = build_html_report(file_name=file_name,metadata=metadata,metrics=metrics,status=status,observations=observations,recommendations=recommendations,)
    st.download_button(label="Descarcă raport HTML",data=report_html,file_name="medical_imaging_qa_report.html",mime="text/html",)
    st.warning("Disclaimer: aplicația este educațională/portofoliu. Nu este dispozitiv medical,"nu oferă diagnostic și nu se folosește pentru decizii clinice.")
